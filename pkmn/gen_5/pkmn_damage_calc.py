...
def calculate_gen_five_damage(
    ...
    if (
        defending_ability not in [gen_five_const.AIR_LOCK_ABILITY, gen_five_const.CLOUD_NINE_ABILITY] and
        attacking_ability not in [gen_five_const.AIR_LOCK_ABILITY, gen_five_const.CLOUD_NINE_ABILITY]
    ):
    ...
    if move.name == gen_five_const.NATURE_POWER_MOVE_NAME:
        ...
        if custom_move_data == gen_five_const.PLAIN_TERRAIN:
            ...
            base_power = 120
            move_type = const.TYPE_WATER
    elif move.name == gen_five_const.WEATHER_BALL_MOVE_NAME and is_weather_active:
        base_power *= 2
